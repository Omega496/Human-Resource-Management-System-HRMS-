...
**[Design Decision]** Reading "a deterministic, cryptographically peppered/salted hash stored
...
explicitly to a human reviewer as the interpretation being built against.
...
- **Fast path:** each request's middleware checks the token's `jti` against a local in-process
  ...
  bulk hydration).
- **[Design Decision] Bulk revocation extension:** for "revoke *all* sessions for user X" (as
  ...
  mechanism described in the source plan, not a replacement for it.
...
2. The bulk-revocation "valid-after watermark" extension in §5 (additive, not required).
3. The interpretation of pepper-storage location in §4.5.
